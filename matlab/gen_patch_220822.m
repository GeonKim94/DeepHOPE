...
for iter_set = 1:length(dir_set)
    ...
    for iter_cls = 2:length(dir_cls)%1:length(dir_cls)
        ...
        for iter_stitch = length(dir_mat):-1:1
            ...
            if y_crop_(1) == 1
                ...
                if y_crop_(2) == size_ori(2)
                    ...
                    if num_crop_y*size_crop(2) > size_ori(2)
                        tomo_stitch = cat(1,tomo_stitch,13370*ones(size(tomo_stitch,2),ceil(num_crop_y*size_crop(2))-size(tomo_stitch,2),size(tomo_stitch,3)));
                        mask_stitch = cat(1,mask_stitch,zeros(size(mask_stitch,2),ceil(num_crop_y*size_crop(2))-size(mask_stitch,2)));
                    end
                end
            else
                if y_crop_(2) == size_ori(2)
                    num_crop_y = ceil((y_crop_(2)-y_crop_(1)+1)/size_crop(2)/stride)*stride;
                    if num_crop_y*size_crop(2) > size_ori(2)
                        tomo_stitch = cat(1,13370*ones(size(tomo_stitch,1), ceil(num_crop_y*size_crop(2))-size(tomo_stitch,2),size(tomo_stitch,3)),tomo_stitch);
                        mask_stitch = cat(1,zeros(size(mask_stitch,1), ceil(num_crop_y*size_crop(2))-size(mask_stitch,2)),mask_stitch);
                    end
                else
            ...
            tomo_stitch = cenpad2d(tomo_stitch,size(tomo_stitch,1)+10,size(tomo_stitch,2)+10,13370);
            mask_stitch = cenpad2d(mask_stitch,size(tomo_stitch,1)+10,size(tomo_stitch,2)+10,0);
