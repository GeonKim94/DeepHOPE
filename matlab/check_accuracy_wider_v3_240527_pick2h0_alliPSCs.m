...
for line_ = unique(lines_test(lines_test>0))'
        ...
        for target_ = [11 7 8 9 10 3 4 5 6 0 1 2]
...
idx_cls_rearrange = [11 7 8 9 10 3 4 5 6 0 1 2];
...
for iter_data = 1:size(preds_test,1)
    if targets_test(iter_data) == -1
        continue
    elseif targets_test(iter_data) > 11
        continue
    end
...
lines_RA = [1 13 14 16 17 18 20 21 22];%unique(lines_test);%[26 27 28 29 30 31 32 33];
%lines_RA(lines_RA<8) = [];
