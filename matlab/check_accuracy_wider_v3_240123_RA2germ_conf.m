...
%             type_ = 5;%3;%4;
    ...
    elseif contains(fname, {'.A2.', 'A2_'})
        lines_test(iter_data) = 39;%7;
        types_test(iter_data) = 8;
    elseif contains(fname, {'.A12.', 'A12_'})
        lines_test(iter_data) = 40;%8;
        types_test(iter_data) = 8;
...
dir_plot = '/data02/gkim/stem_cell_jwshin/data_present/240123_classification_RA2germ';
...
for line_ = unique(lines_test(find((lines_test<12).*(lines_test>0))))'
        if line_ == 0 
            continue
        end
        ...
        num_cummul = [0];
        for target_ = 0:3%sort(unique(targets_test)', 'ascend')
            %ecto meso endo ctl
            num_cummul = [num_cummul num_cummul(end)+sum(y_==target_)];
        end
        num_cummul(1) = [];
        for target_ = 3:-1:0%sort(unique(targets_test)', 'descend')
...
xticks(unique(lines_test(find((lines_test<12).*(lines_test>0)))));
