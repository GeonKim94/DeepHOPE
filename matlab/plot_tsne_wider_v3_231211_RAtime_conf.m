...
dir_out = [dir_out0 '/' ...
    '23_SEC1H5_wider_v3_24h_lateJAX_onRA_fishdeep1_b012_in_lr0.001'];
...
feats_tsne_ = tsne(feats_test,'Perplexity',  perp, 'Standardize',true,'LearnRate',250);


%% if 12h is type 1.5


% for type_ = unique(types_test(find((types_test>0).*(types_test <4))))'
for line_ = unique(lines_test(find((lines_test>0).*(lines_test<10))))'
% 
%     if line_ == 0 
%         continue
%     end
...
%     end
    ...
    plot(feats_tsne_(idxs_thisline,1),feats_tsne_(idxs_thisline,2), marker_,...
        'MarkerSize',mkrsize_, 'Color', color_)
...
[reduction, umap, clusterIdentifiers, extras]=run_umap(feats_test(types_test<3,:),...
...
for type_ = unique(types_test(find((types_test>0).*(types_test <3))))'
...
idxs_thistype = find(types_test == type_);
    ...
    plot(reduction(idxs_thistype,1),reduction(idxs_thistype,2), '.',...
        'MarkerSize',15, 'Color', color_)
