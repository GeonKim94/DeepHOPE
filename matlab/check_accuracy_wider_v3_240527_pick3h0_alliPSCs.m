...
saveas(gcf,[dir_plot, '/', 'prediction_alliPSCs', '.fig']);
saveas(gcf,[dir_plot, '/', 'prediction_alliPSCs', '.png']);
...
saveas(gcf,[dir_plot, '/', 'prediction_normalized_alliPSCs', '.fig']);
saveas(gcf,[dir_plot, '/', 'prediction_normalized_alliPSCs', '.png']);


%% confusion matrix ALL
close all
conf_all = zeros(length(unique(targets_test)),size(scores_test,2));
idx_cls_rearrange = [9 6 7 8 3 4 5 0 1 2 10 11 12 13];
...
for iter_data = 1:size(preds_test,1)
    ...
    conf_all(targets_test(iter_data)+1,preds_test(iter_data)+1) = ...
        conf_all(targets_test(iter_data)+1,preds_test(iter_data)+1)+1;
    
end

conf_all = conf_all(idx_cls_rearrange+1,idx_cls_rearrange(1:size(conf_all,2))+1);

conf_all_norm = conf_all./repmat(sum(conf_all,2), [1 size(conf_all,2)]);

figure(52)
imagesc(conf_all_norm), colormap magma, axis image
saveas(gcf,[dir_plot, '/', 'confusion_GL', '.fig']);
...
idx_cls_rearrange = [9 6 7 8 3 4 5 0 1 2]
%[11 7 8 9 10 3 4 5 6 0 1 2];
%get_reordering_indices([11 3 4 5 6 7 8 9 10 0 1 2],0:11)-1;%[4 2 3 1 0];%[2 3 1 0 4];%[13 9 10 11 12 5 6 7 8 0 1 2 3 4]

for iter_data = 1:size(preds_test,1)
    if targets_test(iter_data) == -1
        continue
    elseif targets_test(iter_data) > 9
        continue
    end
...
lines_RA = [1 11 12 14 15 16 18 18 20];%unique(lines_test);%[26 27 28 29 30 31 32 33];
%lines_RA(lines_RA<8) = [];
...
saveas(gcf,[dir_plot, '/', 'confusion_RA', '.fig']);
...
saveas(gcf,[dir_plot, '/', 'confusion_iPSCs', '.fig']);


%% confusion GMGL
close all
conf_GMGL = zeros(size(scores_test,2),size(scores_test,2));
...
for iter_data = 1:size(preds_test,1)
    ...
    if lines_test(iter_data) > 10
        continue
    end


    conf_GMGL(targets_test(iter_data)+1,preds_test(iter_data)+1) = ...
        conf_GMGL(targets_test(iter_data)+1,preds_test(iter_data)+1)+1;
    
end

conf_GMGL = conf_GMGL(idx_cls_rearrange+1,idx_cls_rearrange+1);

conf_GMGL_norm = conf_GMGL./repmat(sum(conf_GMGL,2), [1 size(conf_GMGL,2)]);

figure(55)
imagesc(conf_GMGL_norm), colormap magma, axis image
saveas(gcf,[dir_plot, '/', 'confusion_GMGL', '.fig']);

%% confusion GMRA
close all
lines_select = [1 11 12];
conf_GMRA = zeros(length(lines_select),size(scores_test,2));
...
for iter_data = 1:size(preds_test,1)
    ...
    if ~ismember(lines_test(iter_data),lines_select)
        continue
    end


    conf_GMRA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1) = ...
        conf_GMRA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1)+1;
    
end

conf_GMRA = conf_GMRA(:,idx_cls_rearrange+1);

conf_GMRA_norm = conf_GMRA./repmat(sum(conf_GMRA,2), [1 size(conf_GMRA,2)]);

figure(55)
imagesc(conf_GMRA_norm), colormap magma, axis image
saveas(gcf,[dir_plot, '/', 'confusion_GMRA', '.fig']);

%% confusion H9RA
close all
lines_select = [14 15 16];
conf_H9RA = zeros(length(lines_select),size(scores_test,2));
...
for iter_data = 1:size(preds_test,1)
    ...
    conf_H9RA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1) = ...
        conf_H9RA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1)+1;
    
end

conf_H9RA = conf_H9RA(:,idx_cls_rearrange+1);

conf_H9RA_norm = conf_H9RA./repmat(sum(conf_H9RA,2), [1 size(conf_H9RA,2)]);

figure(55)
imagesc(conf_H9RA_norm), colormap magma, axis image
saveas(gcf,[dir_plot, '/', 'confusion_H9RA', '.fig']);

%% confusion JXRA
close all
lines_select = [18 19 20];
conf_JXRA = zeros(length(lines_select),size(scores_test,2));
...
for iter_data = 1:size(preds_test,1)
    ...
    conf_JXRA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1) = ...
        conf_JXRA(find(lines_select == lines_test(iter_data)),preds_test(iter_data)+1)+1;
    
end

conf_JXRA = conf_JXRA(:,idx_cls_rearrange+1);

conf_JXRA_norm = conf_JXRA./repmat(sum(conf_JXRA,2), [1 size(conf_JXRA,2)]);

figure(55)
imagesc(conf_JXRA_norm), colormap magma, axis image
saveas(gcf,[dir_plot, '/', 'confusion_JXRA', '.fig']);
