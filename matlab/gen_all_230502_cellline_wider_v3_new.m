...
% new difference: feature updated as newfeat + save masks
...
% dir_source = '/workspace01/gkim/stem_cell_jwshin/data/240716_TCF';
% dir_feat = '/workspace01/gkim/stem_cell_jwshin/data/23_feat_wider_v3_allh_onRA_new/240723_newfeat_wider_v3'; 
% dir_mask = '/workspace01/gkim/stem_cell_jwshin/data/23_mask_wider_v3_allh_onRA/240723_mask_wider_v3'; 
% dir_mip = '/workspace01/gkim/stem_cell_jwshin/data/23_MIPH5_wider_v3_testfl'; 
% dir_sec1 = '/workspace01/gkim/stem_cell_jwshin/data/23_SEC1H5_wider_v3_testfl'; 
% dir_img = '/workspace01/gkim/stem_cell_jwshin/data/23_SEC1PNG_wider_v3_testfl';


dir_source = '/workspace01/gkim/stem_cell_jwshin/';
dir_feat = '/workspace01/gkim/stem_cell_jwshin/data/23_feat_wider_v3_allh_onRA_new/240618_newfeat_wider_v3'; 
dir_mask = '/workspace01/gkim/stem_cell_jwshin/data/23_mask_wider_v3_allh_onRA/240618_mask_wider_v3'; 
dir_mip = '/workspace01/gkim/stem_cell_jwshin/data/24_MIPH5_wider_v3_allh_onRA'; 
dir_sec1 = '/workspace01/gkim/stem_cell_jwshin/data/24_SEC1H5_wider_v3_allh_onRA'; 
dir_img = '/workspace01/gkim/stem_cell_jwshin/data/24_SEC1PNG_wider_v3_allh_onRA';

cd(dir_source)
dir_set = dir('*_TCF');
dir_set = dir_set([dir_set.isdir]);
...
%             tomo_stitch = h5read(fname, sprintf('/Data/3D/%06d', hour));
            ...
            if false% str2num(dir_source(idx_slash+1:idx_slash+6)) < 230701
                tomo_stitch = h5read(fname, sprintf('/Data/3D/%06d', hour));
            ...
            se = strel('disk', round(10/res_ori(1)));
            mask_3D_ = mask_3D.*repmat(imerode(mask_stitch,se), [1,1,size(mask_3D,3)]);
            map_thick = sum(single(mask_3D),3);
            
            X = [];
            Y = [];
            Z = [];
            map_zmax = getzmaxmap(mask_3D_);
            for iter_x = 10:10:size(map_zmax,1)
                for iter_y = 10:10:size(map_zmax,2)
                    if map_zmax(iter_x,iter_y) > 0
                        X = [X; iter_x];
                        Y = [Y; iter_y];
                        Z = [Z; map_zmax(iter_x,iter_y)];
                    end
                end
            end


            [cx, cy, cz, radius] = spherefit(X,Y,Z, res_ori(1), res_ori(2), res_ori(3));

            curvature = -sign(cz)*1/radius;
            ...
            stats = regionprops(single(mask_stitch),map_thick,'WeightedCentroid');
            coor_cot = stats.WeightedCentroid;
            coor_cot = fliplr(coor_cot);     
            ...
            map_dm = sum(single(tomo_stitch)-single(n_m).*((mask_3D-mask_lip_3D)>0) ,3);
            map_dm(map_dm<0) = 0;
            stats = regionprops(single(mask_stitch),map_dm,'WeightedCentroid');
            coor_cod = stats.WeightedCentroid;
            coor_cod = fliplr(coor_cod);     


            map_lip = sum(mask_lip_3D ,3);
            stats = regionprops(single(mask_stitch),map_lip,'WeightedCentroid');
            coor_col = stats.WeightedCentroid;
            coor_col = fliplr(coor_col);     
            ...
            RR = sqrt((XX-coor_cot(1)).^2+(YY-coor_cot(2)).^2);
            spread_thick = sqrt(sum(map_thick(mask_stitch).*(RR).^2)/sum(map_thick(mask_stitch)));
            skew_thick = [(sum(map_thick(mask_stitch).*(XX - coor_cot(1)).^3)/sum(map_thick(mask_stitch)))...
                (sum(map_thick(mask_stitch).*(YY - coor_cot(2)).^3)/sum(map_thick(mask_stitch)))]...
                /spread_thick^3;
            kurt_thick = (sum(map_thick(mask_stitch).*(RR ).^4)/sum(map_thick(mask_stitch)))...
                /spread_thick^4;

            RR = sqrt((XX-coor_cod(1)).^2+(YY-coor_cod(2)).^2);
            ...
            RR = sqrt((XX-coor_col(1)).^2+(YY-coor_col(2)).^2);
            ...
            save([dir_feat, '/',...
                ...
                'spread_lip', 'skew_lip', 'kurt_lip', 'curvature');
            ...
            mask_stitch = logical(mask_stitch);
            size_crop = logical(mask_cyto);
            mask_gap = logical(mask_gap);
            mask_lip = logical(mask_lip);
            mask_boundin = logical(mask_boundin);
            mask_boundout = logical(mask_boundout);
