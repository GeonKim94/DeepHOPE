...
for iter_lineex = 1:length(str_lineex)
    for iter_hour = 1:length(str_hour)% 3 to be done later
        if contains(str_lineex{iter_lineex}, 'no')
            dir_out = sprintf('/data02/gkim/stem_cell_jwshin/outs/230811+230502_MIPH5_wider_v2_%sh_%s_onRA_b002_in_lr0.001',...
        ...
        elseif contains(str_lineex{iter_lineex}, 'on')
            ...
            dir_out = sprintf('/data02/gkim/stem_cell_jwshin/outs/230811+230502_MIPH5_wider_v2_%sh_onRA_b002_in_lr0.001',...
                str_hour{iter_hour});%;, str_lineex{iter_lineex});
            dir_out_gen = 'Nothing';
        end
        ...
        for iter_h = 1:3
            ...
            confusion(targets_test(idxs_data)+1, 1) = confusion(targets_test(idxs_data)+1, 1) + sum(preds_test(idxs_data)==0);
            confusion(targets_test(idxs_data)+1, 2) = confusion(targets_test(idxs_data)+1, 2) + sum(preds_test(idxs_data)==1);
        end
        acc = trace(confusion)/sum(sum(confusion))
        ...
        for iter_h = 1:length([unique(lines_test)]')
            ...
            confusion_genp(targets_test(idxs_data)+1, 1) = confusion_genp(targets_test(idxs_data)+1, 1) + sum(preds_test(idxs_data)==0);
            confusion_genp(targets_test(idxs_data)+1, 2) = confusion_genp(targets_test(idxs_data)+1, 2) + sum(preds_test(idxs_data)==1);
        end
        acc_genp = trace(confusion_genp)/sum(sum(confusion_genp))
        %%
        h9 = figure(9);
        set(h9, 'Color', [1 1 1])
