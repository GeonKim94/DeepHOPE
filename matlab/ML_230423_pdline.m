...
clc
close all
dir_src = ('/data02/gkim/stem_cell_jwshin/data/230306_feature_v3/00_train');
        ...
        cd(dir_line(iter_line).name)
            ...
            for iter_feat = 1:length(dir_feat)
                ...
                elseif contains(dir_line(iter_line).name, 'A20')
                    cond = 2; % ctl  
                elseif contains(dir_line(iter_line).name, 'A15')
                    cond = 3; % RA continue%
                elseif contains(dir_line(iter_line).name, 'E5')
                    cond = 11; % RA  
                elseif contains(dir_line(iter_line).name, 'E3')
                    cond = 12; % E6
                elseif contains(dir_line(iter_line).name, 'A4')
                    cond = 21; % ctl  
...
cd '/data02/gkim/stem_cell_jwshin/data/230306_feature_v3'
for iter_fea = 1:size(features_,2)
    h_ = figure(iter_fea);
...
idxs_test = [find(cond_ == 3); find(cond_ == 12); find(cond_ == 23)];
