%%
[feats_hc_pc, score] = pca(feats_hc', 'NumComponents', 10);
save([dir_plot '/hc_pc.mat'], 'feats_hc_pc');    
    ...
    num_dot = num_dot+length(idx_data);
        ...
        scatter(feats_hc_pc(idx_data,1),feats_hc_pc(idx_data,2),marker_,...
        'SizeData', size_, 'MarkerEdgeColor', [1 1 1], 'MarkerFaceColor', color_plot,...
        'MarkerFaceAlpha', 1,'MarkerEdgeAlpha', 1)
    %     plot(feats_hc_pc(idxs_thisline,1),feats_hc_pc(idxs_thisline,2), marker_,...
    %     'MarkerSize', size_, 'MarkerEdgeColor', [0 0 0], 'MarkerFaceColor', color_)
        
    
    end
    
    ycen = min(feats_hc_pc(:,2))/2+max(feats_hc_pc(:,2))/2;
    xcen = min(feats_hc_pc(:,1))/2+max(feats_hc_pc(:,1))/2;
    yax = max(feats_hc_pc(:,2))-min(feats_hc_pc(:,2));
    xax = max(feats_hc_pc(:,1))-min(feats_hc_pc(:,1));
    
        ylim([min(feats_hc_pc(:,2)) max(feats_hc_pc(:,2))]);
        xlim([min(feats_hc_pc(:,1)) max(feats_hc_pc(:,1))]);
    
    set(gcf,'color',[1 1 1])
    axis image
    axis off
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_notime.fig', perp)])
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_notime.png', perp)])
    ...
    axis image
    axis off
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_endo.fig', perp)])
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_endo.png', perp)])
    ...
    axis image
    axis off
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_meso.fig', perp)])
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_meso.png', perp)])
    ...
    axis image
    axis off
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_ecto.fig', perp)])
    saveas(h_fig, [dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_ecto.png', perp)])
    ...
    for idx_feat_hc = 1:size(feats_hc,2)%[3 15 16 20 21 23 25 32 33]% [1 3 5 13 15 16 20 21 24 25 30 32 33] %[1 3 13 15 16 ] %1:length(names_feat_hc)
        ...
        close all
        if isfile([dir_plot '/' sprintf('pc_onlyGLGM_hc_p%03d_%s.fig', perp, names_feat_hc{idx_feat_hc})])
            continue
        end
        ...
        axis image
        axis off
        saveas(h_fig, [dir_feat '/' sprintf('pc_onlyGLGM_hc_p%03d_%s.fig', perp, names_feat_hc{idx_feat_hc})])
        saveas(h_fig, [dir_feat '/' sprintf('pc_onlyGLGM_hc_p%03d_%s.png', perp, names_feat_hc{idx_feat_hc})])
    end