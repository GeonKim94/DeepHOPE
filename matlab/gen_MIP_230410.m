...
dir_source = '/data02/gkim/stem_cell_jwshin/data/230407_TCF'; % 220822_TCF';%220715_TCF';
dir_save = '/data02/gkim/stem_cell_jwshin/data/230407_MIP'; %220902_3D';%220803_3D';
dir_img = '/data02/gkim/stem_cell_jwshin/data/230407_PNG';%%230221_PNG'; %220902_PNG';%220803_PNG';
...
%             end
            ...
            if y_crop_(1) == 1
                ...
                if y_crop_(2) == size_ori(2)
                    ...
                    if num_crop_y*size_crop(2) > size_ori(2)
                        tomo_stitch = cat(2,tomo_stitch,13370*ones(size(tomo_stitch,1),ceil(num_crop_y*size_crop(2))-size(tomo_stitch,2),size(tomo_stitch,3)));
                        mask_stitch = cat(2,mask_stitch,zeros(size(mask_stitch,1),ceil(num_crop_y*size_crop(2))-size(mask_stitch,2)));
                    end
                end
            else
                if y_crop_(2) == size_ori(2)
            ...
            cd([dir_img, '/',...
                dir_set(iter_set).name, '/',...
                dir_cls(iter_cls).name]);
            save([fname_save, sprintf('_hr_%02d.mat', hour)],...
                'data','resx','resy','resz','fname','-v7.3');
