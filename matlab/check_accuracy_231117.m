...
for iter_lineex = 1:length(str_lineex)
    for iter_hour = 1:length(str_hour)% 3 to be done later
        ...
        dir_out_genh = [dir_out sprintf('_test%s', 'HD')];
        cd(dir_out);
        ...
        if ~exist(dir_out_genh)
        else
        cd(dir_out_genh)
        list_model = dir(['epoch[' epoch_bestmodel_str '*]']);
        dir_bestmodel_genh = list_model(1).name;
        cd(dir_bestmodel_genh)
        
        load('result_test.mat');

        idxs_fakedata = find(targets == 0);
        ...
        paths_test = paths';
        targets_test = single(targets');
        paths_test(idxs_fakedata,:) = [];
        scores_test(idxs_fakedata,:) = [];
        feats_test(idxs_fakedata,:) = [];
        targets_test(idxs_fakedata,:) = [];
        ...
        for iter_data = 1:length(targets_test)
            ...
            if contains(fname, '.HD02.') || contains(fname, 'HD02_')
                lines_test(iter_data) = 1;
        ...
        %%
        h27 = figure(27);
        set(h27, 'Position', [0 200 1600 240])
        set(h27, 'Color', [1 1 1])
        ...
        %%
        h28 = figure(28);
        set(h28, 'Position', [0 200 900 300])
        set(h28, 'Color', [1 1 1])
...
%         h29 = figure(29);
%         set(h29, 'Color', [1 1 1])
%         set(h29, 'Position', [0 200 400 400])
%         rocObj = rocmetrics(targets_test',...
%             reshape(scores_test(:,2) - scores_test(:,1), size(targets_test')),1);
%         plot(rocObj)
        ...
        %%
        if save_fig
            saveas(h27, [dir_out_genh '_' 'figbest_profile.fig'])
            saveas(h28, [dir_out_genh '_' 'figbest_inference.fig'])
%             saveas(h29, [dir_out_genb '_' 'figbest_ROC.fig'])
        end
        if save_png
            saveas(h27, [dir_out_genh '_' 'figbest_profile.png'])
            saveas(h28, [dir_out_genh '_' 'figbest_inference.png'])
%             saveas(h29, [dir_out_genb '_' 'figbest_ROC.png'])
        end
