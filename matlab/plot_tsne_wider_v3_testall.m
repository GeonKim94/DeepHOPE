...
dir_out = [dir_out0 '/' ...
    '230811+230502_SEC1H5_wider_v3_allh_noJAX_onRA_fishdeep1_b008_in_lr0.001'];
...
%%
feats_tsne_ = tsne(feats_test(types_test<5,:), 'Perplexity', 50, 'Standardize',true,'LearnRate',250);
%% if 12h is just 2
...
for line_ = unique(lines_test)'
    ...
    plot(feats_tsne_(idxs_thisline,1),feats_tsne_(idxs_thisline,2), '.',...
...
idxs_thistype = find(types_test == type_);
    ...
    plot(feats_tsne_(idxs_thistype,1),feats_tsne_(idxs_thistype,2), '.',...
        'MarkerSize',15, 'Color', color_)
