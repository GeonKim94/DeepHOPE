...
dir_source = '/data02/gkim/stem_cell_jwshin/data/230306_TCF'; % 220822_TCF';%220715_TCF';
dir_save = '/data02/gkim/stem_cell_jwshin/data/230306_3DH5'; %220902_3D';%220803_3D';
dir_img = '/data02/gkim/stem_cell_jwshin/data/230306_MIPXYPNG';%%230221_PNG'; %220902_PNG';%220803_PNG';
...
%             mask_stitch = (mask_stitch==label_colony);
            ...
            count_z = squeeze(squeeze(sum(sum(single(data>1.342),1),2)));
            z_glass = gradient(count_z);
            z_glass = find(z_glass == max(z_glass));
           
            if z_glass-2 < 1
                data = cat(3, 1.337*ones(size(data,1),size(data,2),2-z_glass+1),data );
                z_glass = 3;
            end
            ...
            if (size(data,3)-z_glass+2) >= round(thickness_min/res_new_(3))
                ...
                data = cat(3, data, 1.337*ones(size(data,1),size(data,2), round(thickness_min/res_new_(3)) - (size(data,3)-z_glass+2)));
...
%                 'data','-v7.3');
            ...
            subplot(2,1,1), imagesc(squeeze(max(data,[],1))', [1.3370 1.3800]), axis image, colormap gray
            daspect([3 1 1])
            
            
            subplot(2,1,2), imagesc(squeeze(max(data,[],2))', [1.3370 1.3800]), axis image, colormap gray
            daspect([3 1 1])
            ...
            cd([dir_img, '/',...
                dir_set(iter_set).name, '/',...
                dir_cls(iter_cls).name]);
            saveas(h_, ['00_colony_' fname_save, '_MIPXY.png'])
        end
    end
