...
dir_source = '/data02/gkim/stem_cell_jwshin/data/220812_TCF';%220715_TCF';
dir_save = '/data02/gkim/stem_cell_jwshin/data/220812_3D_';%220803_3D';
dir_img = '/data02/gkim/stem_cell_jwshin/data/220812_FIG_';%220803_PNG';
...
for iter_set = 1:length(dir_set)
    ...
    for iter_cls = 1:length(dir_cls)
        ...
        for iter_stitch = 1:length(dir_mat)
            ...
            size_new = [320 320 128];
            res_new_ = [0.15 0.15 0.15];
            ...
            size_pad = round(size_new(1)/2);%20/res_ori(1); %(um / res)
            x_crop_ = find(sum(mask_stitch,2)>0);
            x_crop_ = [max(min(x_crop_)-size_pad,0) min(max(x_crop_)+size_pad,size_ori(1))];
            y_crop_ = find(sum(mask_stitch,1)>0);
            y_crop_ = [max(min(y_crop_)-size_pad,0) min(max(y_crop_)+size_pad,size_ori(2))];
            ...
            for iter_x = 1:num_crop(1)/stride-1
                for iter_y = 1:num_crop(2)/stride-1
...
%                         dir_cls(iter_cls).name]);
%                     
                    data = imresize3(tomo,size_new, 'cubic'); %%'linear'
...
%                     
                    ...
                    title('MIP - XZ')
                    subplot(2,2,4), imagesc(mipz_stitch, [13370 13800]), axis image, colormap gray
                    ...
                    saveas(h, [fname_save, sprintf('_%02d_%02d.fig',iter_x, iter_y)])
