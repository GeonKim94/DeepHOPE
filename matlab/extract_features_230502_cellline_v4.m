...
dir_source = '/data02/gkim/stem_cell_jwshin/data/230502_TCF';%220822_TCF';
dir_save = '/data02/gkim/stem_cell_jwshin/data/230502_feature_v3';
cd(dir_source);
dir_set = dir('0*');

hours = 0;%[0:4:24, 1:4:24, 2:4:24, 3:4:24];
for hour = hours
    for iter_set = 1:6%length(dir_set)
        cd(dir_source);
        ...
        for iter_cls = 1:length(dir_cls)
            ...
            dir_tcf = dir('23*');
                ...
                for iter_tcf = 1:length(dir_tcf)
                    ...
                    tomo_temp = h5read(fname, sprintf('/Data/3D/%06d', hour));
                    ...
                    stats = regionprops(mask_mip, 'MajorAxisLength', 'MinorAxisLength', 'Eccentricity', 'Circularity');
                    
                    majax = stats.MajorAxisLength *resx;
                    minax = stats.MinorAxisLength *resx;
                    eccen = stats.Eccentricity;
                    circu = stats.Circularity;
                    ...
                    mask_mipv2 = bwlabeln(mask_mipv2);
                    ...
                    stats = regionprops(single(mask_mip),'Centroid');
                    coor_crd = stats.Centroid;
                    coor_crd = fliplr(coor_crd);      

                    stats = regionprops(single(mask_mip),sum(single(tomo_temp-13370)/10000,3), 'WeightedCentroid');
...
%                         sum(YY.*map_thick(mask_mip))/sum(map_thick(mask_mip))];
                    ...
                    mask_nuc = (n_low<13450).*(n_low>13400);
                    se2 = strel('disk',1);
                    mask_nuc = imerode(mask_nuc,se2);
                    mask_nuc = imdilate(mask_nuc,se2);
                    mask_nuc = mask_nuc.*mask_interior;

                    mask_gap = n_low<=13400;
                    mask_gap = imerode(mask_gap,se2);
                    mask_gap = imdilate(mask_gap,se2);
                    mask_gap = mask_gap.*mask_interior;
...
%                     set(gca, 'Colormap',gray)
                    ...
                    mask_3D_lip = mask_3D.*(tomo_temp>13675);
%                     se4 = strel('sphere',4);
%                     mask_3D_lip = mask_3D_lip - imdilate(imdilate(imerode(mask_3D_lip,se4),se4),se4);
%                     mask_3D_lip(mask_3D_lip<0) = 0;
                    mask_3D_lip = mask_3D_lip.*repmat(mask_lip,[1,1,size(tomo_temp,3)]);
                    [label_lip, num_lip] = bwlabeln(mask_3D_lip,6);

                    mask_3D_nolip = mask_3D-(mask_3D_lip);
                    mask_3D_nolip(mask_3D_nolip<0) = 0;

                    %dmdensity_mean = (mean(single(tomo_temp(find(mask_3D-nolip)))/10000)-1.337)/0.18*100;
                    
                    vol_colony = sum(sum(sum(mask_3D)))*resx*resy*resz;
                    vol_lipid = sum(sum(sum(mask_3D_lip)))*resx*resy*resz;
                    RI_mean_nolip = mean(single(tomo_temp(find(mask_3D_nolip)))/10000);
                    RI_mean_lip = mean(single(tomo_temp(find(mask_3D_lip)))/10000);

                    area_col = sum(sum(mask_mip))*resx*resy;
                    area_nuc = sum(sum(mask_nuc))*resx*resy;
                    area_gap = sum(sum(mask_gap))*resx*resy;
    ...
    %                 set(gca, 'Colormap',gray)
                    %sprintf('_hr%02d.mat',hour)
                    save(strrep(fname, '.TCF','.mat'), 'smoothness', 'ncont_bound',...
                        'area_col', 'area_nuc', 'area_lip','num_lip','area_gap',...
                        'thick_mean','thick_std','thick_peak',...
                        'center_disp', 'spread_thick', 'skew_thick', 'kurt_thick', ...
                        'RI_mean', 'RI_std', 'majax', 'minax', 'eccen', 'circu',...
                        'RI_mean_nolip', 'RI_mean_lip', 'vol_colony', 'vol_lipid');
            end
        end
    end
end
