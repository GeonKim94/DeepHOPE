...
str_lineex = {'GM', 'H9', 'JAX'};
str_hour = {'12', '24', 'all'};
cmap_line = [1 1 0; 1 0 1; 0 1 1];
for iter_lineex = 1:3
for iter_hour = 1:3% 3 to be done later

dir_out = sprintf('/data02/gkim/stem_cell_jwshin/outs/230502_MIPH5_wider_%sh_no%s_b002_in_lr0.001_2',...
    str_hour{iter_hour}, str_lineex{iter_lineex});
dir_out_gen = [dir_out sprintf('_test%s', str_lineex{iter_lineex})];
...
for iter_data = 1:length(targets_test)
    ...
    if contains(fname, 'GM') || contains(fname, 'gm')
        lines_test(iter_data) = 1;
    elseif contains(fname, 'H9') || contains(fname, 'h9')
