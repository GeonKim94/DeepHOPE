...
dir_case = '23_SEC1H5_wider_v3_allh0_GM_germline_fishdeep10_b012_in_lr0.001_fix';
dir_out = [dir_out0 '/' ...
    dir_case];
...
for iter_data = 1:length(targets_test)
    ...
    elseif contains(fname, {'.A2.', 'A2_'})
        lines_test(iter_data) = 28;%7;
        types_test(iter_data) = 6;
    elseif contains(fname, {'.A12.', 'A12_'})
        lines_test(iter_data) = 29;%8;
        types_test(iter_data) = 6;
...
colors_line = [[3/4 3/4 3/4]%'GM untreated',%1
        ...
        [1/2 1/2 1/2]%'HD 11',%27
        [1/2 1/2 1/2]%'PD 02',%@8
        [1/2 1/2 1/2]%'PD 12',%29
        [1/2 1 0]%]%'HD 18',%30
        [1/2 1 0]%'BJ 13',%31
        [1/2 1 0]%'PD 19',%32
        [1/2 1 0]%'PD 20',%33
...
%%
[colors_line_,idx_clr2cls,idx_line2clr] = unique(colors_line, 'row');
%colors_line_(idx_line2clr(line_)) = wanted color
% colors_line_(idx_line2clr,:) = colors_line(idx_clr2cls,:)
% in [C,ia,ic] = unique(A,'rows','legacy')
% it suffices that C = A(ia,:) & A = C(ic,:)
...
for idx_color = 1:length(colors_line_)
    ...
    if length(idx_data) > 40
        ...
        order_shuffle_sample = order_shuffle(1:40);
        idx_data_sample = idx_data(oreder_shuffle_sample);
    end
...
%     sprintf('The number of data for line # %d is %d', line_, length(idxs_thisline))
    ...
    h_fig.Color = [1 1 1];
    hold on
    idxs_thisline = find(lines_test_ == line_);
...
lines_test_ = lines_test(lines_test>0);
...
%%
for line_ = unique(lines_test_)'
    sprintf('number of data for line %02d: %03d', line_, sum(lines_test_==line_))
end
...
%         idxs_thisline = randsample(idxs_thisline, 50);
    ...
    scatter(coefficients(idxs_thisline,1),coefficients(idxs_thisline,2),marker_,...
    'SizeData', size_, 'MarkerEdgeColor', [1 1 1], 'MarkerFaceColor', color_,...
    'MarkerFaceAlpha', 1,'MarkerEdgeAlpha', 1)
%     plot(feats_tsne_(idxs_thisline,1),feats_tsne_(idxs_thisline,2), marker_,...
%     'MarkerSize', size_, 'MarkerEdgeColor', [0 0 0], 'MarkerFaceColor', color_)
...
axis image
axis off
saveas(h_fig, [dir_plot '/' 'PCA.fig'])
saveas(h_fig, [dir_plot '/' 'PCA.png'])
