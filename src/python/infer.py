...
get_dloader = lambda dset: None if len(dset) == 0 else DataLoader(dset, batch_size=config['optim']['batch_size'],
...
runner.infer(dloader_train,savefile=True)
runner.infer(dloader_val,savefile=True)
runner.infer(dloader_test,savefile=True)