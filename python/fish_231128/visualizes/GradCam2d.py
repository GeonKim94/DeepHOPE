...
class GradCam2d:
    def __init__(self, model, name_part, idx_layer, device):#(self, model, hooks, device):
        ...
        self.device = device
        self.model = model.to(device)  # Avoid deep copy to save memory
        self.model.eval()
...
#            hook_layer = hook_layer._modules[h]
        ...
        if 'layer' in self.model._modules[name_part][idx_layer]._modules.keys():
            if 'layer' in self.model._modules[name_part][idx_layer]._modules['layer']._modules.keys():
                hook_layer = self.model._modules[name_part][idx_layer]._modules['layer']._modules['layer'][0]
            else:
                hook_layer = self.model._modules[name_part][idx_layer]._modules['layer'][0]
        else:
            hook_layer = self.model._modules[name_part][idx_layer]._modules['0']
...
#        output.backward(gradient=one_hot, retain_graph=True)
    ...
    def gdcam(self, grad, feature):
        ...
        #print('shape of grad: {}'.format(grad.shape))
        weights = F.adaptive_avg_pool2d(grad, 1)
        weights.squeeze_(0) 
        #print('shape of xy-averaged weight squeezed: {}'.format(weights.shape))
        ...
        gcam = gcam.detach().cpu().numpy()
        del feature, grad, weights
        return gcam
    
    def gdcampp(self, grad, feature):
        grad = torch.unsqueeze(grad, 0)
        grad_2 = torch.mul(grad,grad)
        grad_3 = torch.mul(grad,grad_2)
        weights = torch.div(grad_2, ((2 * grad_2) + torch.mul(grad_3, feature) + 1e-12))
        # grad_2 = grad * grad
        # grad_3 = grad * grad_2
        # weights = grad_2 / ((2 * grad_2) + (grad_3 * feature) + 1e-12)
        
        #print('shape of individual weight: {}'.format(weights.shape))
        
        weights = F.adaptive_avg_pool2d(weights, 1)
        #print('shape of xy-averaged weight: {}'.format(weights.shape))
        weights.squeeze_(0)
    ...
    def layercam(self, grad, feature):
        #print(feature.shape)
        #print(grad.shape)
        #weights = F.adaptive_avg_pool2d(grad, 1)
        #weights.squeeze_(0)

        gcam = torch.mul(feature,grad).sum(dim=0)
        #print(gcam.shape)
    ...
    def __call__(self, input_, idx=None, mode="gdcam"):
        #with torch.no_grad():
        ...
        if mode == "gunho":
            ...
            gcam = self.layercam(grad, feature_map)
    ...
    def multicam(self, input_, idx=None):
        ...
        # score_weight = []
        # b, c, h, w = input_.size()
        # k, u, v = feature_map.size() # b, k, u, v 
        # score_saliency_map = torch.zeros((1, 1, u, v))#.to(self.device)#torch.zeros((1, 1, h, w)).to(self.device)
        # for i in range(k):
        #     saliency_map = torch.unsqueeze(torch.unsqueeze(feature_map[i,:,:].cpu(),0),1)#torch.unsqueeze(feature_map[:,i,:,:],1)
        ...
        #     norm_saliency_map = F.interpolate(norm_saliency_map, size = (h,w), mode = 'bilinear', align_corners = False)
        #     output = self.model(input_.to(self.device) * norm_saliency_map.to(self.device))
        #     output = output[0]
        #     output = output.cpu()
        #     output = F.softmax(output)
        #     print(output)
        #     score = output[0][pred_idx]
        #     score_saliency_map += score * saliency_map.cpu()
        #     score_weight.append(score.detach().cpu().numpy())#append(score.detach().cpu().numpy())
        ...
        return resp, gd, gdpp, pred_idx#, score_saliency_map
    @staticmethod
