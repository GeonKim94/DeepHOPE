...
def _bn_relu_conv(in_c, out_c, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True), **conv_kwargs):
...
class _ConvBlock(nn.Module):
    ...
    """
    def __init__(self, in_c, out_c, stride=1, dilation=1, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
        super().__init__()
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_c, out_c, shortcut=lambda x: x,
                 stride=1, dilation=1,
                 norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
...
class TransferBlock(nn.Module):
    ...
    """
    def __init__(self, ch, num_blk, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
        super().__init__()
...
class DownRefinementBlock(nn.Module):
    ...
    Forwarding Path:
                    ??     (BN-ReLU-Conv)     ??        input image - (ConvBlock) * num_blk -(sum)- feature - (MaxPool) - output
    """
    def __init__(self, in_c, out_c, num_blk, stride=1, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
        super().__init__()
...
class UpRefinementBlock(nn.Module):
    ...
    Forwarding Path:
                    ??  (Channel Reduction)    ??        input image - (ConvBlock) * num_blk -(sum)- feature - (UpSample) - output
    """
    def __init__(self, in_c, out_c, num_blk, stride=1, dilation=1, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
        super().__init__()
        self.k = in_c // out_c
