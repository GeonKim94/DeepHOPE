...
from models.layers.switcablenorm import SwitchNorm2d
from models.layers.fish_module_2d import FishHead, FishBody, FishTail, Bridge

# https://github.com/kevin-ssy/FishNet/blob/master/models/fishnet.py#L197
def _conv_bn_relu(in_ch, out_ch, stride=1, norm=nn.BatchNorm2d, act=lambda : nn.ReLU(True)):
    return nn.Sequential(nn.Conv2d(in_ch, out_ch, kernel_size=3, padding=1, stride=stride, bias=False),
                         norm(out_ch),
                         act())

class Fishnet_2d(nn.Module):
    """
    ...
    """
    def __init__(self, input_c = 1, start_c=64, num_cls=1000, norm="bn", act="relu",
        ...
        self.stem = nn.Sequential(
            _conv_bn_relu(input_c, start_c//2, stride=2, norm=norm, act=act),
    ...
    def forward(self, x):
        ...
        feature = self.classifier[0].forward(head_features[-1])
        feature = self.classifier[1].forward(feature)
        feature = self.classifier[2].forward(feature)
        feature = self.classifier[3].forward(feature)
        feature = self.classifier[4].forward(feature)
        feature = self.classifier[5].forward(feature)
        
        #import pdb; pdb.set_trace()
        return out, feature #None
...
def fish99_2d(input_c, num_classes=1000):
    start_c = 64
    ...
    return Fishnet_2d(input_c, start_c, num_classes, 
                   tail_num_blk, bridge_num_blk,
                   body_num_blk, body_num_trans,
                   head_num_blk, head_num_trans,
                   **net_channel)

def fish150_2d(input_c, num_classes=1000, norm="bn", act="relu"):
    start_c = 64
    ...
    return Fishnet_2d(input_c, start_c, num_classes, norm, act,
                   tail_num_blk, bridge_num_blk,
                   body_num_blk, body_num_trans,
                   head_num_blk, head_num_trans,
                   **net_channel)

def fishdw_2d(input_c, num_classes=1000, norm="bn", act="relu"):
    start_c = 16
...
def fishdw2_2d(input_c, num_classes=1000, norm="bn", act="relu"):
    start_c = 16
...
def fishdeep1_2d(input_c, num_classes=2, norm="in", act="lrelu"):#,
                    # start_c,
                    # tail_num_blk, bridge_num_blk,
                    # body_num_blk, body_num_trains,
                    # head_num_blk, head_num_trans):
    start_c = 16
    tail_num_blk = [2, 2, 4, 4, 8, 8]
    bridge_num_blk = 4

    body_num_blk = [2, 2, 2, 2, 2, 2]
    body_num_trans = [2, 2, 2, 2, 2, 2]

    head_num_blk = [2, 2, 2, 2, 2, 4]
    head_num_trans = [2, 2, 2, 2, 2, 4]
...
def fishdeep10_2d(input_c, num_classes=2, norm="in", act="lrelu"):#,
                    # start_c,
                    # tail_num_blk, bridge_num_blk,
                    # body_num_blk, body_num_trains,
                    # head_num_blk, head_num_trans):
    start_c = 16
    tail_num_blk = [1, 1, 2, 2, 4, 4]
    bridge_num_blk = 2

    body_num_blk = [1, 1, 1, 1, 1, 1]
    body_num_trans = [1, 1, 1, 1, 1, 1]

    head_num_blk = [1, 1, 1, 1, 1, 2]
    head_num_trans = [1, 1, 1, 1, 1, 2]
...
def fishdeep2_2d(input_c, num_classes=2, norm="in", act="lrelu"):#, # for chlee embryoo 1024*1024
                    # start_c,
                    # tail_num_blk, bridge_num_blk,
                    # body_num_blk, body_num_trains,
                    # head_num_blk, head_num_trans):
    start_c = 32
    tail_num_blk = [2, 2, 4, 4, 6]
    bridge_num_blk = 2

    body_num_blk = [1, 1, 1, 1, 1]
    body_num_trans = [1, 1, 1, 1, 1]

    head_num_blk = [1, 1, 2, 2, 3]
    head_num_trans = [1, 1, 1, 1, 4]
