...
if __name__ == "__main__":
    ...
    target_shape = crop_shape + (size_z,) # (2048,2048,12)
    if arg.dim == "2d":
    ...
    if arg.test is False:
        ...
        if arg.patchval:
            val_pdsets = TomoPatchLoader.getdatasets(path = os.path.join(arg.data_dir, "val"), patch_size = target_shape,
                                                        transform = tform_test, test = True, aug_rate = 0,
                                                        pats_exclude = arg.pats_exclude,pats_class = arg.pats_class,
                                                        reset_class = arg.reset_class, mode_class = arg.mode_class)
            test_pdsets = TomoPatchLoader.getdatasets(path = os.path.join(arg.data_dir, "test"), patch_size = target_shape,
                                                        transform = tform_test, test = True, aug_rate = 0,
                                                        pats_exclude = arg.pats_exclude,pats_class = arg.pats_class,
                                                        reset_class = arg.reset_class, mode_class = arg.mode_class)
        else:
        ...
        if arg.dim == "3d":
            ...
            if arg.patchval:
                test_loader = torch.utils.data.DataLoader(test_pdsets, 1, num_workers = arg.cpus, worker_init_fn = worker_init_fn)
                #for input_, target_, inputDir_, coor_off_ in enumerate(loader_test):#enumerate(loader_test):
                #    break
                input_, target_, inputDir_, coor_off_ = next(iter(test_loader))
        ...
        if arg.reset_class:
            ...
            if arg.patchval:
                value_list = [value for value in test_pdsets[2].values()]
            else:
        ...
        if len(idx_front) > 1:
            for order_model in range(len(idx_comma)):
                ...
                model = TomoRunner(arg, net, optim, torch_device, loss, logger, fname_temp,
                            w_metric = arg.w_metric, w_metric_test = arg.w_metric_test, w_metric_train = arg.w_metric_train)
                print("model defined")
                print(arg.w_metric)
