...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
	...
	default_df_dict.update({
		...
		# seg params
		'seg_level': np.full((total), int(seg_params['seg_level']), dtype=np.int8), # sampling level for segmentation
		'sthresh': np.full((total), int(seg_params['sthresh']), dtype=np.uint8), # segmentation thres
		# 'mthresh': np.full((total), int(seg_params['mthresh']), dtype=np.uint8), # median blurring thres
