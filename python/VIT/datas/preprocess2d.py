...
def pad_2d(img, target_shape, **kwargs):
    origin_size = img.shape
    if any(xi > yi for xi, yi in zip(target_shape[:2], img.shape[:2])):

        if target_shape[0]>img.shape[0]:
            pad_xf = math.floor((target_shape[0]-img.shape[0])/2)
            pad_xb = target_shape[0]-img.shape[0] - pad_xf
        else:
            pad_xf = 0
            pad_xb = 0
        if target_shape[1]>img.shape[1]:
            pad_yf = math.floor((target_shape[1]-img.shape[1])/2)
            pad_yb = target_shape[1]-img.shape[1] - pad_yf
        else:
            pad_yf = 0
            pad_yb = 0
        pad_width = ((pad_xf, pad_xb), (pad_yf, pad_yb))
        if img.dtype == 'uint16':
            pad_val = 13370
        else:
            if abs(13370-(np.mean(img[:20,:20])+np.mean(img[-20:,-20:])+np.mean(img[:20,-20:])+np.mean(img[-20:,:20]))/4) < 200:
                pad_val = 13370
            elif np.max(img) <= 1:
                pad_val = 0
            else:
...
def random_crop_2d(img, target_shape=crop_shape, **kwargs):
    ...
    if origin_size[0] > target_shape[0]:
        ...
        if len(img.shape)>2:
            array_pad_front = padval*np.ones((pad_len_front,img.shape[1],img.shape[2]))
        else:
            array_pad_front = padval*np.ones((pad_len_front,img.shape[1]))

        if len(img.shape)>2:
            array_pad_back = padval*np.ones((pad_len_back,img.shape[1],img.shape[2]))
        else:   
            array_pad_back = padval*np.ones((pad_len_back,img.shape[1]))
        img = np.concatenate((array_pad_front,
            img, array_pad_back), axis = 0)
    ...
    if origin_size[1] > target_shape[1]:
        ...
        if len(img.shape)>2:
            array_pad_front = padval*np.ones((img.shape[0],pad_len_front,img.shape[2]))
        else:
            array_pad_front = padval*np.ones((img.shape[0],pad_len_front))
        
        if len(img.shape)>2:
            array_pad_back = padval*np.ones((img.shape[0],pad_len_back,img.shape[2]))
        else:   
            array_pad_back = padval*np.ones((img.shape[0],pad_len_back))
        img = np.concatenate((array_pad_front,
            img, array_pad_back), axis = 1)
