...
def mat2npy(mat, **kwargs):
    ...
    if np.max(img) > 10000:
        ...
        ri = 1.337#mat['ri'] # ys kim data (evenly mixed data)
    #ri = 1.33
    return img, ri

def calibration(img, ri = None):
    if np.max(img) > 10000:
        cap_min = 13300
        cap_max = 14000
    else:
...
def pad_3d(img, target_shape, **kwargs):
    origin_size = img.shape
    if any(xi > yi for xi, yi in zip(target_shape, img.shape)):
        ...
        if target_shape[1]>img.shape[1]:
            ...
            pad_yf = 0
            pad_yb = 0
        if target_shape[2]>img.shape[2]:
            pad_zf = math.floor((target_shape[2]-img.shape[2])/2)
            pad_zb = target_shape[2]-img.shape[2] - pad_zf
        else:
            pad_zf = 0
            pad_zb = 0
        ...
        if img.dtype == 'uint16':
            ...
            if abs(13370-(np.mean(img[:20,:20])+np.mean(img[-20:,-20:])+np.mean(img[:20,-20:])+np.mean(img[-20:,:20]))/4) < 200:
                ...
                pad_val = 1.337
        img = np.pad(img, mode = 'constant', constant_values=pad_val)
    return img
...
def random_crop_3d(img, target_shape=crop_shape, **kwargs):
    ...
    if origin_size[2] > size_z:
        ...
        pad_len_front = random.randint(0, pad_len) 
        pad_len_back =  pad_len - pad_len_front
        img = np.concatenate((padval*np.ones((img.shape[0],img.shape[1],pad_len_front)),
            img, padval*np.ones((img.shape[0],img.shape[1],pad_len_back))), axis = 2)
