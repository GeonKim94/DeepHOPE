...
class clsRunner:
    def __init__(self,  net, optim, torch_device, loss,  load_fname = None, reset_loss = 0, model_type = "fishdeep10", epoch = 500,
                 save_dir = '/workspace01/gkim/stem_cell_jwshin/outs/temp', result_dir = '/workspace01/gkim/stem_cell_jwshin/outs/temp', 
        ...
        self.writer = SummaryWriter(log_dir = save_dir + "/log_TB")

        if False:#arg.optim == "sgd":
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optim, 16) #32
        else:
            self.scheduler = None
    ...
    def load(self, filename=None):
        ...
        if filename is None:
            ...
            if len(filenames) == 0:
                ...
                filename = os.path.basename(filenames[-1]) #self.arg.testfile
        ...
        for epoch in range(self.start_epoch, self.epoch):
            ...
            for i, (input_, target_, path) in enumerate(train_loader):
                ...
                if i == len(train_loader)-1:
                    self.writer.add_scalar('Accuracy/train', loss_sum/targets.shape[0], epoch)
                    #pass #self.logger.log_write("train", epoch=epoch, loss=loss_sum/targets.shape[0])
...
#             false_f = open(self.result_dir + "/false.txt", "w")
        ...
        for i, (input_, target_, inputDir_) in enumerate(loader):
            ...
            if "ViT" in self.model_type:
                output_ = self.net(input_)
                feat_ = np.zeros(0, dtype=np.object)
            else:
...
#######################################################################
    ...
    def _get_acc_onepatch(self, loader):
        ...
        max_score, pred_avg = np.max(scores_avg, axis = 1)
    ...
    def _get_acc_patches(self, datasets):
        ...
        loader_test = torch.utils.data.DataLoader(dataset, 1, num_workers = 1)
        ...
        for dataset in datasets:
            loader = torch.utils.data.DataLoader(dataset, 1, num_workers = 0)
            #import pdb; pdb.set_trace()
    ...
    def test_patch(self, val_pdsets, test_pdsets, train_pdsets = None):
        print('testing...')
        dataset = test_pdsets[0]

        loader_test = torch.utils.data.DataLoader(dataset, 1, num_workers = 1)
        idx, (input_, target_, inputDir_, coor_off_) = next(enumerate(loader_test))
        if "ViT" in self.model_type:
            output_ = self.net(input_)
            feat_ = np.zeros(0, dtype=np.object)
            feat_shape = 0
        else:
            output_, feat_ = self.net(input_.to(self.torch_device))
            feat_shape = feat_.shape[1]
        score_shape = output_.shape[1]

        coor_shape = len(coor_off_) # coor_off_.shape[1]
        ...
        path = self.fname[0:13] + "train[%.4f]_valid[%.4f]_test[%.4f]_patchval" % (acc_train_avg, acc_avg, acc_test_avg)
    ...
    def eval_pdsets(self, pdsets, feat_shape, coor_shape, score_shape): # get accuracy of all data (a list of pdsets)
        self.net.eval()
        with torch.no_grad():
            targets = np.zeros(0, dtype=np.int8)
            preds_avg = np.zeros(0, dtype=np.int8)
            ...
            for dataset in pdsets:
                loader = torch.utils.data.DataLoader(dataset, 1, num_workers = 1)
                ...
                msg_test = "[Testing progress: {0:0.3f}%".format((count_dset+1)/len(pdsets)*100)
                print('\r' + msg_test, end='', flush=True)
                count_dset += 1
        ...
        print('dataset length: {}'.format(correct_vote.shape))    
        print('dataset length: {}'.format(correct_avg.shape))    
        print('number of correct colonies (vote): {}'.format(np.sum(correct_vote)))
        print('number of correct colonies (avg): {}'.format(np.sum(correct_avg)))
        return targets, scores, paths, feats, coors_off, correct_avg, correct_vote, acc_avg, acc_w_avg, acc_vote, acc_w_vote
        



    def eval_patches(self, loader): # infer patches from the given loader of a single data
        if loader is None:  
            return None, None, None, None, None
        targets = np.zeros(0, dtype=np.int8)
        scores = np.zeros((0,self.n_class), dtype=np.float32)
        ...
        if pred_vote.size>1:
            pred_vote = pred_vote(np.argmax(scores_avg[0][pred_vote], axis = 1))
        #import pdb;pdb.set_trace()
        #print('Data name: {}, ground truth: {}, prediction_avg: {}, prediction_vote: {}'.format(os.path.split(inputDir_[0])[1], loader.dataset.classes[targets[0]], loader.dataset.classes[pred_avg[0]], loader.dataset.classes[pred_vote]))
        return targets, scores, paths, feats, coors_off, pred_avg, pred_vote


    def valid_patch(self, epoch, val_pdsets, test_pdsets, acc_train, acc_train_w, train_loader = None): #validation in patcheval setting
        print('testing...')
        dataset = val_pdsets[0]
        matdict = {}

        loader_test = torch.utils.data.DataLoader(dataset, 1, num_workers = 1)
        idx,item = next(enumerate(loader_test))
        input_, target_, inputDir_, coor_off_ = item
        if "ViT" in self.model_type:
            output_ = self.net(input_)
            feat_ = np.zeros(0, dtype=np.object)
        else:
        ...
        with torch.no_grad():
            ...
            if acc_avg > self.best_metric * 0.98:
                bool_save = True
            if acc_w_avg > self.best_metric_weighted * 0.98:
                bool_save = True
