...
def random_crop_2d(img, target_shape=crop_shape, **kwargs):
    ...
    if origin_size[0] > target_shape[0]:
        ...
        img = np.concatenate((1.337*np.ones((pad_len_front,img.shape[1])),
            img, 1.337*np.ones((pad_len_back,img.shape[1]))), axis = 0)
    ...
    if origin_size[1] > target_shape[1]:
        ...
        pad_len_front = random.randint(0, pad_len) 
        pad_len_back =  pad_len - pad_len_front
        img = np.concatenate((1.337*np.ones((img.shape[0],pad_len_front)),
            img, 1.337*np.ones((img.shape[0],pad_len_back))), axis = 1)
