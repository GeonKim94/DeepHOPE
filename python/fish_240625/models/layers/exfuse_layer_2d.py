...
class CNA(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=3, stride=1, padding=1,
        ...
        self.layer = nn.Sequential(nn.Conv32d(in_c, out_c, kernel_size, stride, padding),
                                   norm(out_c), act(True))
...
class SEB_dw(nn.Module):
    def __init__(self, low_feature, high_feature,
                 norm=nn.BatchNorm2d, up_scale=2):
...
class GCN(nn.Module):
    def __init__(self, in_c, out_c, ks=7, norm=nn.BatchNorm2d):
...
class ECRE(nn.Module):
    def __init__(self, in_c, up_scale=2, norm=nn.BatchNorm2d):
